    ...
    function showCurrentPage() {
        ...
        setTimeout(() => {
            ...
            eventsToShow.forEach((event, index) => {
                ...
                if (!template) {
                    console.warn('[displaySelectedLeagueEvents] Plantilla event-item-template no encontrada');
                    return;
                }
                const div = template.content.cloneNode(true).querySelector('.event-item');
                ...
                const homeImg = div.querySelector('.home-logo');
                homeImg.src = homeLogo;
                if (homeLogo) homeImg.classList.remove('hidden');
                const awayImg = div.querySelector('.away-logo');
                awayImg.src = awayLogo;
                if (awayLogo) awayImg.classList.remove('hidden');
                div.querySelector('[data-datetime]').textContent = eventDateTime + statusText;
                div.querySelector('[data-stadium]').textContent = `Estadio: ${event.estadio || 'Por confirmar'}`;
                if (isInProgress) {
...
function clearTeamData(type) {
    ...
    if (box) {
        const stats = box.querySelectorAll('[data-stat]');
        stats.forEach(stat => {
            const statName = stat.dataset.stat;
            if (statName === 'pj' || statName === 'points' || statName === 'dg') {
                stat.textContent = `PJ: 0`; // Ejemplo para general; ajustar según sección
            } else if (statName === 'pjHome' || statName === 'winsHome' || statName === 'dgHome') {
                stat.textContent = `PJ: 0`;
            } else if (statName === 'pjAway' || statName === 'winsAway' || statName === 'dgAway') {
                stat.textContent = `PJ: 0`;
            }
        });
    }
...
function fillTeamData(teamName, leagueCode, type) {
    ...
    if (box) {
        const stats = {
            ...
            dg: dg >= 0 ? '+' + dg : dg || 0,
            pjHome: t.pjHome || 0,
            winsHome: t.winsHome || 0,
            dgHome: dgHome >= 0 ? '+' + dgHome : dgHome || 0,
            pjAway: t.pjAway || 0,
            winsAway: t.winsAway || 0,
            dgAway: dgAway >= 0 ? '+' + dgAway : dgAway || 0
        };
        for (const [key, value] of Object.entries(stats)) {
            const statElement = box.querySelector(`[data-stat="${key}"]`);
            if (statElement) {
...
function dixonColesProbabilities(tH, tA, league) {
    ...
    if (allData.calendario && allData.calendario[ligaName]) {
        ...
        const expectedDrawRate = 0.25;
        rho = -0.1 * (drawRate / expectedDrawRate);
        rho = Math.max(-0.2, Math.min(-0.05, rho));
